...
$fh = fopen($basePath . '/voteData/2018-107年地方公職人員選舉/直轄市區域議員/elpaty.csv', 'r');
$party = array();
...
$fh = fopen($basePath . '/voteData/2018-107年地方公職人員選舉/直轄市區域議員/elcand.csv', 'r');
$ref = array();
while ($line = fgetcsv($fh, 2048)) {
    foreach ($line AS $k => $v) {
        $line[$k] = trim($v, '\'');
...
$fh = fopen($basePath . '/voteData/2018-107年地方公職人員選舉/直轄市區域議員/elctks.csv', 'r');
$result = array();
while ($line = fgetcsv($fh, 2048)) {
    foreach ($line AS $k => $v) {
        $line[$k] = trim($v, '\'');
...
$header = array('省市別', '縣市別', '選區別', '鄉鎮市區', '村里別', '號次', '名字', '政黨代號', '性別', '出生日期', '年齡', '出生地', '學歷', '現任', '當選註記', '副手');
$fh = fopen($basePath . '/voteData/2018-107年地方公職人員選舉/縣市區域議員/elcand.csv', 'r');
while ($line = fgetcsv($fh, 2048)) {
    foreach ($line AS $k => $v) {
        $line[$k] = trim($v, '\'');
    }
...
$header = array('省市別', '縣市別', '選區別', '鄉鎮市區', '村里別', '投開票所', '候選人號次', '得票數', '得票率', '當選註記');
$fh = fopen($basePath . '/voteData/2018-107年地方公職人員選舉/縣市區域議員/elctks.csv', 'r');
while ($line = fgetcsv($fh, 2048)) {
    foreach ($line AS $k => $v) {
        $line[$k] = trim($v, '\'');
    }
