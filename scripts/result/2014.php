...
$voteDataPath = $basePath . '/voteData/2014-103年地方公職人員選舉';
...
$candidateFiles = [
    ...
    $voteDataPath . '/村里長/elcand.csv' => '村里長',
];
...
foreach ($candidateFiles as $candidateFile => $election) {
    ...
    while ($line = fgetcsv($fh, 2048)) {
        ...
        if (!isset($area[$code])) {
            $code = $line[0] . $line[1] . '00' . $line[3] . $line[4];
        }
        if (!isset($votes[$vcode])) {
            $vcode = $line[0] . $line[1] . '00' . $line[3] . $line[4] . '0' . $data['cand_no'];
        }
        switch ($election) {
            case '直轄市山地原住民區民代表':
            case '直轄市山地原住民區長':
